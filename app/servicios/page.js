...
export const metadata = {
  ...
  openGraph: {
    ...
    description: 'Software a la medida, redes, datos y consultoría para empresas en Colombia.',
    ...
    <>
      ...
      <section className="relative pt-32 pb-16 bg-gray-950 overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-6 text-center">
          <p className="text-verde-400 font-bold text-xs uppercase tracking-[0.3em] mb-4">Lo que hacemos</p>
          <h1 className="font-display text-5xl md:text-6xl font-extrabold text-white tracking-tighter mb-6">
            Nuestros Servicios
          </h1>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            Cuatro áreas de especialización que cubren todo el ciclo tecnológico de tu empresa,
            desde la infraestructura hasta la inteligencia de negocio.
          </p>
        </div>
      </section>
            ...
            >
              <div>
                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 ${
                  i % 2 === 0 ? 'bg-verde-100 text-verde-600' : 'bg-white/20 text-white'
                }`}>
                  {iconMap[s.id]}
                </div>
                <h2 className={`font-display text-3xl font-extrabold tracking-tighter mb-4 ${
                  i % 2 === 0 ? 'text-gray-950' : 'text-white'
                }`}>
                  {s.titulo}
                </h2>
                <p className={`text-lg leading-relaxed ${i % 2 === 0 ? 'text-gray-500' : 'text-verde-100'}`}>
                  {s.descripcion}
                </p>
              </div>
              <div>
                ...
                <ul className="space-y-3">
                  {detalles[s.id].bullets.map((b) => (
                    <li key={b} className="flex items-start gap-3">
                      <svg className={`w-5 h-5 mt-0.5 shrink-0 ${i % 2 === 0 ? 'text-verde-500' : 'text-verde-200'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                      </svg>
                      <span className={`text-sm ${i % 2 === 0 ? 'text-gray-600' : 'text-verde-50'}`}>{b}</span>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <section className="py-16 bg-gray-50 text-center">
        <div className="mx-auto max-w-xl px-6">
          ...
          <p className="text-gray-500 mb-8">Escríbenos y te respondemos en menos de 24 horas.</p>
          <Link
            href="/contacto"
